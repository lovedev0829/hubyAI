...
const Login: React.FC<LoginProps> = () => {
  ...
  const slackRedirectUriLogin: string =
    `${currentEnvironment}.slackRedirectUriLogin`
      .split(".")
      .reduce((obj: any, key: string) => obj?.[key], config);

  const slackOAuthUrl = `https://slack.com/oauth/v2/authorize?client_id=${slackClientId}&scope=${slackScope}&user_scope=${slackUserScope}&redirect_uri=${slackRedirectUriLogin}`;
          ...
          const response = await axios.post(
            ...
            null,
            {
              params: {
                ...
                code: code,
                redirect_uri: slackRedirectUriLogin,
              },
            }
          );
  ...
  const handleOffice365Login = async () => {
    try {
      ...
      // console.log(isVerified, redirectUrl, "isVerified and redirectUrl values");
      // if (isVerified === "V") {
  ...
  const logIn = async () => {
    ...
    try {
      ...
      if (token_data) {
        const { access_token, refresh_token } = token_data.data;
        // const accessTokenExpire = new Date().getTime() + 15  60  1000;
        // const refreshTokenExpire = new Date().getTime() + 24  60  60 * 1000;
        localStorage.setItem("access_token", JSON.stringify({ access_token }));
        localStorage.setItem(
          "refresh_token",
          JSON.stringify({ refresh_token })
      ...
      const getUser_email = await getUserDataByEmail(email, access_user);
      const userData = getUser_email.data[0];
      localStorage.setItem("userData", JSON.stringify(userData));
      // const expiryTime = new Date().getTime() + 15  60  1000;
      // localStorage.setItem("expiryTime", expiryTime.toString());
      // const isVerified = userData.verification;
  ...
  const oauthLogin = useGoogleLogin({
    onSuccess: async (e: any) => {
      try {
        ...
        // console.log(googleLoginData, "4544445454");
        const loginresponse = await fetchPostApi(
          "/api/users/login",
          googleLoginData,
          "application/json",
        ...
        if (loginresponse.data) {
          ...
          const access_user = JSON.parse(localStorage.getItem("access_token"));
          const getUser_email = await getUserDataByEmail(
      ...
      <div className="">
        ...
        <div>
          ...
          <div className="flex justify-between">
            ...
            <Link
              to={"/forgotpassword"}
              className="mt-[8px] no-underline text-[16px] font-semibold text-[#6495ED]"
            >
              Forgot Password
            </Link>
          </div>
        ...
        <div className="flex sm:flex-row flex-col items-center gap-[20px]">
          ...
          <button className="sm:w-full w-[50%]" onClick={handleOffice365Login}>
            <img
              src={office365}
              alt="office365"
        ...
        <p className="text-[#696969] font-semibold sm:text-[18px] text-[15px] mt-[20px]">
          ...
          <a href="/privacy" className="text-[#553dcd] underline">
            Privacy Policy
          </a>
          .
        </p>
